...
- **Input control over a separate DataChannel, without touching the signaling protocol**: `RTCDataChannel` is automatically negotiated inside the existing offer/answer SDP exchange — no new WebSocket message type is needed; the host's offer-creation ordering (unchanged since Phase 1) supports this naturally.
- **Input authorization reuses existing device pairing approval, with no separate step**: telling an already-screen-visible device "but you can't control it" has little practical value in the own-devices-to-own-devices use case; a deliberately accepted risk (see the Phase 2 design spec, §0 Security).
